...
for char = ['A':'H','J']
    ...
    parfor i=1:num_samples
        ...
        [FA_SVD, residual_FA_SVD(i)] = registration(A,a);
        [FB_SVD, residual_FB_SVD(i)] = registration(B,b);
        d_set(:,i) = FB\FA*[a_tip';1];
    end
    FA_set = [FA_set; sum(residual_FA)];
    FB_set = [FB_set; sum(residual_FB)];
    residual_FA_set = [residual_FA_set; sum(residual_FA)];
    residual_FB_set = [residual_FB_set; sum(residual_FB)];
    residual_FA_SVD_set = [residual_FA_SVD_set; sum(residual_FA_SVD)];
    residual_FB_SVD_set = [residual_FB_SVD_set; sum(residual_FB_SVD)];
